...
export function predictEmployee(row:any):EmployeePrediction{
  ...
  const digital=num(row["Skill_Level"] ?? row["Digital Readiness"] ?? row["Digital Skill"],60);
  const training=num(row["Training_History"] ?? row["Training_History"] ?? row["Training Hours"],20);
